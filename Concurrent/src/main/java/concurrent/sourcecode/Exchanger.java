...
public class Exchanger<V> {
     ...
     * concurrent.sourcecode.TimeoutException} is thrown.  If the time is less than or equal
     ...
     * @throws InterruptedException if the current thread was
     *         interrupted while waiting
     * @throws concurrent.sourcecode.TimeoutException if the specified waiting time elapses
     *         before another thread enters the exchange
     */
    public V exchange(V x, long timeout, java.util.concurrent.TimeUnit unit)
        throws InterruptedException, concurrent.sourcecode.TimeoutException {
        if (!Thread.interrupted()) {
            ...
            if (v != CANCEL)
                return (V)v;
            if (!Thread.interrupted())
                throw new concurrent.sourcecode.TimeoutException();
        }
        throw new InterruptedException();
    }
}
